// FILE: src/app/ui/cards/ProductCardUI.jsx
// Pure UI component — receives raw product object from productsData
// Data shape: product.title, product.discount, product.dimensions, product.images (array of {color, hex, images:[]})
      ...
      {Array.from({ length: 10 }).map((_, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            animation: `hpburst${i} 0.6s ease-out forwards`,
          }}>
            <style>{`
              @keyframes hpburst${i} {
    ...
    <div style={{ display: "flex", alignItems: "center", gap: "2px" }}>
      {[1, 2, 3, 4, 5].map((s) => (
        <Star key={s} size={11} style={{
          fill:  s <= Math.round(rating) ? "var(--color-gold)" : "transparent",
          color: s <= Math.round(rating) ? "var(--color-gold)" : "#D1D5DB",
...
export default function ProductCardUI({
  ...
  // ── Derive display values from new productsData shape ──────────────────────
  ...
  // Cart product shape expected by AddToCartButton
  const cartProduct = {
    id:            product.id,
    name:          product.title,
  ...
  return (
    <div
      ...
      style={{
        background: "var(--color-white)", borderRadius: "20px", overflow: "hidden",
    ...
    >
      ...
      <div style={{ padding: "12px 12px 13px", display: "flex", flexDirection: "column", flexGrow: 1 }}>
        ...
        <h3 style={{
          ...
          color: "var(--color-text-primary)", marginBottom: "8px", lineHeight: 1.4,
        }}>
          {product.title}
        </h3>
