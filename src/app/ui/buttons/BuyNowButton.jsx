...
import BuyNowSheet from "../sheet/BuyNowSheet";
...
export default function BuyNowButton({
  ...
  fullWidth = true,
  className = "",
  mode      = "direct",  // 🆕 "direct" = add to cart + redirect | "cart" = show sheet
  product   = null,      // 🆕 required when mode="direct"
}) {
  ...
  const handleClick = () => {
    if (mode === "cart") {
      // Cart page — show WhatsApp confirmation sheet
      setOpen(true);
      return;
    }

    // Product page — add to cart then redirect
    if (product) {
      ...
      if (!exists) {
        ...
        window.dispatchEvent(new Event("cartUpdate")); // 🆕 update badge
      }
    }

    setClicked(true);
    setTimeout(() => {
      router.push("/cart-items"); // 🆕 redirect to cart
      setClicked(false);
    }, 300);
  };
