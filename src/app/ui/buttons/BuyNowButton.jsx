        ...
        <div className="px-6 pb-10 pt-3">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-[16px] font-bold text-blue-dark">Send Order on WhatsApp</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex flex-col gap-2.5 mb-6">
            {[
              { n: "1", text: "WhatsApp opens with your full order pre-filled" },
...
function ProductOrderSheet({ open, onClose, product, price, mrp }) {
  ...
  const handleConfirm = () => {
    const message = [
      ...
      (product.category && product.slug) ? `  Link       : ${origin}/${product.category}/${product.slug}` : null,
      ``,
      `━━━━━━━━━━━━━━━━━━━━━━━━━━`,
      `💰 *Pricing*`,
      ...
      appliedCoupon ? `  Coupon     : ${appliedCoupon.code} (−${appliedCoupon.pct}% = −₹${discountAmt.toLocaleString()})` : null,
  ...
  return (
    <>
      ...
      <div
        className="fixed bottom-0 left-0 right-0 z-[71] bg-white rounded-t-2xl shadow-xl"
        style={{ maxWidth: 480, margin: "0 auto" }}
      >
        <div className="flex justify-center pt-3 pb-1">
          <div className="w-10 h-1 rounded-full bg-border" />
        ...
        <div className="px-5 pb-10 pt-2">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-[16px] font-bold text-blue-dark" style={{ fontFamily: "'Playfair Display', serif" }}>
              Order Summary
            </h2>
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-bg-main flex items-center justify-center hover:bg-border transition-colors cursor-pointer">
              <X size={15} className="text-text-secondary" />
            </button>
          </div>
          ...
          <div className="group bg-white hover:bg-bg-main border border-border rounded-2xl shadow-soft p-4 mb-5 transition-all duration-300">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-semibold text-text-primary leading-snug mb-1.5 line-clamp-2"
                ...
                {product.rating && (
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <span className="text-[11px] text-text-secondary">{product.rating} ({product.reviews || 0})</span>
                  </div>
                )}
                {product.selectedColor && (
                  <p className="text-[11px] text-text-secondary mb-1.5">
                    Color: <span className="font-semibold text-text-primary">{product.selectedColor}</span>
                  </p>
                )}
                <div className="flex items-center gap-2">
                  <span className="text-[17px] font-bold text-blue-dark">₹{price.toLocaleString()}</span>
                  {mrp > price && <span className="text-[13px] text-text-secondary line-through">₹{mrp.toLocaleString()}</span>}
                </div>
              </div>
            </div>
            {/* Qty */}
            <div className="flex justify-end mt-3">
              <div className="flex items-center border border-border rounded-xl overflow-hidden bg-white">
                ...
                <span className="w-10 text-center font-bold text-[14px] text-text-primary border-x border-border">{quantity}</span>
          ...
          <div className="flex flex-col gap-3 mb-4">
            <div className="flex items-center justify-between text-[14px]">
              <span className="text-text-secondary">Subtotal (×{quantity})</span>
...
export default function BuyNowButton({
  ...
  return (
    <>
      <button
        onClick={() => { if (!disabled) setSheetOpen(true); }}
        disabled={disabled}
        className={`
          ${fullWidth ? "w-full" : ""}
