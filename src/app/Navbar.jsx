...
// Build nav items: static + one dropdown per category
...
// ─────────────────────────────────────────────────────────────────────────────
// SEARCH DATA — typed suggestion objects built from CATEGORIES + PRODUCTS
// ─────────────────────────────────────────────────────────────────────────────
...
// Small icon/thumbnail per suggestion type
  ...
  return (
    <div className="absolute left-1/2 -translate-x-1/2 top-full pt-3 z-50 min-w-[260px]">
      ...
      <div className="bg-white border border-border rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] overflow-hidden">
        ...
        <Link href={item.href}
          className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-blue-soft to-white border-b border-border group/hdr hover:from-blue-soft hover:to-blue-soft transition-all">
        ...
        {/* View all footer */}
        <Link href={item.href}
          className="flex items-center justify-center gap-1.5 px-4 py-2.5 bg-blue-soft hover:bg-blue transition-all duration-200 group/all">
          <span className="text-[12px] font-bold text-blue group-hover/all:text-white transition-colors">View all {item.label}</span>
...
function NavItem({ item }) {
  ...
  return (
    <div className="relative" onMouseEnter={enter} onMouseLeave={leave}>
      <button className="flex items-center gap-1 text-[13px] font-medium text-text-secondary hover:text-blue transition-colors duration-200 py-1 cursor-pointer">
        {item.label}
...
function DesktopSearch({ open, onClose }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setTimeout(() => setQuery(""), 220);
    }
  }, [open]);

  // Filter suggestions — up to 9 results
  ...
  return (
    <>
      <style>{`
        @keyframes srSlideDown { from { transform:translateY(-100%); opacity:0 } to { transform:translateY(0); opacity:1 } }
      ...
      {/* Panel */}
      <div className="fixed top-0 left-0 right-0 z-[81] bg-white"
        style={{ borderBottom: "1px solid var(--color-border)", boxShadow: "0 8px 40px rgba(0,0,0,0.12)", animation: "srSlideDown 0.25s ease forwards" }}>
        <div className="max-w-3xl mx-auto px-6 py-5">

          {/* ── Input ── */}
          <div className="flex items-center gap-3 rounded-2xl px-5 py-3.5"
            style={{ background: "var(--color-bg-main)", border: "1.5px solid var(--color-border)", transition: "border-color 0.2s, box-shadow 0.2s" }}
            onFocusCapture={(e) => {
              e.currentTarget.style.borderColor = "var(--color-blue)";
              e.currentTarget.style.boxShadow   = "0 0 0 3px rgba(71,87,146,0.1)";
            }}
            onBlurCapture={(e) => {
          ...
          >
            <Search size={19} style={{ color: "var(--color-text-secondary)", flexShrink: 0 }} />
            <input
              ref={inputRef}
              value={query}
            ...
            {query && (
              <button onClick={() => setQuery("")}
                className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center cursor-pointer hover:bg-gray-300 transition-colors shrink-0">
                <X size={12} />
              </button>
            )}
            <button onClick={onClose}
              style={{ fontSize: 13, fontWeight: 600, background: "none", border: "none", cursor: "pointer", marginLeft: 6, color: "var(--color-text-secondary)", flexShrink: 0 }}
          ...
          {/* ── Results / Default ── */}
          <div style={{ marginTop: 12, paddingBottom: 4, maxHeight: "62vh", overflowY: "auto", scrollbarWidth: "none" }}>
              ...
              results.length === 0 ? (
                <div style={{ textAlign: "center", padding: "36px 0" }}>
                  ...
                  <Link href="/products" onClick={onClose}
                    style={{ display: "inline-block", marginTop: 12, fontSize: 13, fontWeight: 700, color: "var(--color-blue)" }}>
                    Browse all products →
                  </Link>
                </div>
              ) : (
                ...
                <div>
                  {recent.length > 0 ? (
                    ...
                    <div style={{ padding: "24px 12px", textAlign: "center" }}>
                      <Clock size={22} style={{ color: "var(--color-border)", margin: "0 auto 8px", display: "block" }} />
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, paddingLeft: 4 }}>
                    {TRENDING_CHIPS.map((t) => (
                      ...
                      >
                        <TrendingUp size={11} style={{ color: "var(--color-gold)" }} />
                        {t.label}
                      </button>
                    ))}
                  </div>
    ...
    <>
      ...
      <div className={`fixed left-0 top-0 h-full w-full bg-white z-[71] flex flex-col transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-bg-main [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {[STATIC_NAV[0], STATIC_NAV[1]].map((item) => (
            <Link key={item.label} href={item.href} onClick={onClose}
              className="flex items-center justify-between px-4 py-3.5 mb-2.5 rounded-[14px] bg-white font-semibold text-[14px] text-blue-dark no-underline hover:bg-blue-soft transition-colors"
          ...
          {CATEGORIES.map((cat) => (
            <div key={cat.slug} className="mb-2.5">
              <button
                ...
                style={{
                  border:     expanded === cat.slug ? "1.5px solid var(--color-blue-dark)" : "1.5px solid var(--color-border)",
                  background: expanded === cat.slug ? "var(--color-blue-dark)" : "var(--color-white)",
                  color:      expanded === cat.slug ? "#fff" : "var(--color-blue-dark)",
                }}>
                {cat.name}
                <ChevronDown size={16} style={{
                  color:      expanded === cat.slug ? "#fff" : "var(--color-blue)",
              ...
              <div className="overflow-hidden transition-all duration-[350ms] ease-[cubic-bezier(0.4,0,0.2,1)]"
                ...
                <div className="mt-2 rounded-xl border border-border bg-white overflow-hidden">
                  ...
                  <Link href={`/${cat.slug}`} onClick={onClose}
                    className="flex items-center gap-3 px-3.5 py-3 border-b border-border hover:bg-blue-soft transition-colors no-underline">
                  ...
                  {cat.subcategories.map((sub) => (
                    <Link key={sub.slug}
                      href={`/products?category=${cat.slug}&sub=${sub.slug}`}
                      onClick={onClose}
                      className="flex items-center gap-3 px-3.5 py-3 border-b border-border last:border-b-0 hover:bg-blue-soft transition-colors no-underline">
          ...
          {[STATIC_NAV[2], STATIC_NAV[3]].map((item) => (
            <Link key={item.label} href={item.href} onClick={onClose}
              className="flex items-center justify-between px-4 py-3.5 mb-2.5 rounded-[14px] bg-white font-semibold text-[14px] text-blue-dark no-underline hover:bg-blue-soft transition-colors"
              style={{ border: "1.5px solid var(--color-border)" }}>
              {item.label}
